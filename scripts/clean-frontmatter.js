...
function getGitLogForFile(filePath) {
    try {
        ...
        const gitCommand = ['log', '--follow', '--pretty=format:%ad|||%s|||%an', '--date=format:%Y-%m-%dT%H:%M%z', filePath];
...
async function main() {
    ...
    for (const arg of args) {
        if (arg.includes('*')) {
            const matched = await glob(arg, { ignore: ['node_modules/**', 'build/**', '.git/**', 'storybook-static/**'] });
            files.push(...matched);
        } else {
            files.push(arg);
        }
