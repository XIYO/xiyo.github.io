...
describe('Category and Post Sorting', () => {
	...
	it('should sort child categories by latest post date', async () => {
		...
		const childCategories = await rootCategory.getChildCategories();

		// 각 카테고리의 최신 포스트 날짜가 제대로 정렬되어 있는지 확인
		for (let i = 0; i < childCategories.length - 1; i++) {
			const currentLatest = await childCategories[i].getLatestPostDate();
			const nextLatest = await childCategories[i + 1].getLatestPostDate();
