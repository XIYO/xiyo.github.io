...
function getLastCommitMarkdownFiles() {
	try {
		// HEAD에서 변경된 마크다운 파일들
		const result = execSync('git diff-tree --no-commit-id --name-only -r HEAD', {
			encoding: 'utf8'
		});
		const files = result
			.split('\n')
			.filter((file) => file.trim() && file.endsWith('.md'))
			.filter((file) => file.startsWith('static/')) // static/ 하위만 적용
			.map((file) => path.resolve(file))
			.filter((file) => fs.existsSync(file)); // 실제 존재하는 파일만 (삭제된 파일 제외)

		console.log(`마지막 커밋에서 변경된 static/ 마크다운 파일: ${files.length}개`);
		return files;
	} catch (error) {
	...
	for (const [key, value] of Object.entries(frontmatter)) {
		if (Array.isArray(value)) {
			...
			for (const item of value) {
				...
				const needsQuotes =
					/[:#\-[\]{}|>*&!%@`]/.test(item) || item.includes(' ') || item.includes('\n');
				if (needsQuotes) {
					yaml += `  - "${item.replace(/"/g, '\\"')}"\n`;
				} else {
					yaml += `  - ${item}\n`;
