...
description: 로컬과 리모트로 `intel`, `macOS 14.0`, `sonoma` 환경에서 `brew`를 사용해 `mongoDB`를 설치하는 과정을 정리합니다.
---
# {{ page.title }}
...
- `mongoDB`는 `cloud` 서비스가 있으며, `free tier`를 제공합니다.  
  `cloud` 서비스를 사용하면 `mongoDB`를 설치할 필요가 없습니다.
- 이 문서에서는 `brew`를 통해 `mongoDB`를 설치합니다.
- [공식 다운로드](https://www.mongodb.com/try/download/community)에서 제공하는 명령어인 `brew install mongodb-atlas`는 `cloud` 사용자를 위한 패키지 입니다.  
  `cloud` 사용자가 아니라면 이 패키지를 설치할 필요는 없습니다.  
  > 🟣 IMPORTANT
  >
